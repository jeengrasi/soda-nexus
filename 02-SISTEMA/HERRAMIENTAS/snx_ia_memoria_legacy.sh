...
mod_memoria() {
  ls -1 "$BASE" > "$MEM/memoria_index.txt" 2>/dev/null || true
}
...
mod_contexto() {
  ...
  if [ ! -f "$FILE" ]; then
    ...
    echo "- Memoria institucional" >> "$FILE"
  fi
}

mod_antiamnesia() {
  if [ ! -d "$BASE" ]; then
    log "ALERTA: falta carpeta de memoria"
  fi
}
...
mod_engranaje() {
  echo "{\"relaciones\":[\"00-GOBIERNO\",\"02-SISTEMA\"]}" > "$MEM/engranaje_${MINISTERIO}.json"
}
...
mod_estado() {
...

  {
    ...
    echo "  \"memoria_index\": \"$( [ -f "$MEM/memoria_index.txt" ] && wc -l < "$MEM/memoria_index.txt" || echo 0 ) entradas\","
    echo "  \"engranaje\": $(cat "$MEM/engranaje_${MINISTERIO}.json"),"
    echo "  \"sync\": $(cat "$MEM/sync_${MINISTERIO}.json"),"
    echo "  \"archivos\": {"
    echo "    \"memoria_index\": \"$( [ -f "$MEM/memoria_index.txt" ] && echo ok || echo missing )\","
