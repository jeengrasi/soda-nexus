#!/usr/bin/env bash
# SNX — PREPARAR ESTRUCTURA PARA CLOUDFLARE PAGES  # OBS: Script para organizar monitor web.

BASE="/data/data/com.termux/files/home/soda"                           # OBS: Ruta soberana.
WEB_DIR="$BASE/03-OPERACIONES/WEB/monitor"                             # OBS: Carpeta para proyecto Pages.
SRC_HTML="$BASE/03-OPERACIONES/SNX-MONITOR.html"                       # OBS: HTML actual del monitor.
DST_HTML="$WEB_DIR/index.html"                                         # OBS: HTML destino como index.html.

mkdir -p "$WEB_DIR"                                                    # OBS: Crea carpeta WEB/monitor dentro de 03-OPERACIONES.
cp "$SRC_HTML" "$DST_HTML"                                             # OBS: Copia el monitor como index.html para Pages.

echo "[SNX-PAGES] Estructura preparada en $WEB_DIR"                    # OBS: Mensaje de confirmación.
