...
# ============================================================================
# SNX — FASE 5.9.3
# IA-SCRIPT CENTRAL — MODO BACKEND V2 (NO INTERACTIVO, VALIDADO, DETERMINISTA)
...
registrar() {
    local mensaje="$1"
    local archivo="$REGISTROS/snx_backend_v2_$(date +"%Y%m%d_%H%M%S").log"
    echo "[$(timestamp)] $mensaje" >> "$archivo"
}

registrar "MODO BACKEND V2: Inicio de ejecución no interactiva."
...
python3 "$BACKEND/monitor_soberano.py" > "$MONITOR_DATA" 2>/tmp/snx_backend_v2.err || {
...
registrar "MODO BACKEND V2 finalizado correctamente."
exit 0
