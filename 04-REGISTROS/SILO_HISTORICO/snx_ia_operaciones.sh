...
mod_memoria() {
  ls -1 "$BASE" > "$MEM/operaciones_tree.txt" 2>/dev/null || true
}
...
mod_contexto() {
  ...
  if [ ! -f "$FILE" ]; then
    ...
    echo "- Operaciones del sistema" >> "$FILE"
  fi
}

mod_antiamnesia() {
  if [ ! -d "$BASE" ]; then
    log "ALERTA: falta carpeta de operaciones"
  fi
}
...
mod_engranaje() {
  echo "{\"relaciones\":[\"02-SISTEMA\",\"04-REGISTROS\"]}" > "$MEM/engranaje_${MINISTERIO}.json"
}
...
mod_estado() {
...

  {
    ...
    echo "  \"operaciones_tree\": \"$( [ -f "$MEM/operaciones_tree.txt" ] && wc -l < "$MEM/operaciones_tree.txt" || echo 0 ) entradas\","
    echo "  \"engranaje\": $(cat "$MEM/engranaje_${MINISTERIO}.json"),"
    echo "  \"sync\": $(cat "$MEM/sync_${MINISTERIO}.json"),"
    echo "  \"archivos\": {"
    echo "    \"operaciones_tree\": \"$( [ -f "$MEM/operaciones_tree.txt" ] && echo ok || echo missing )\","
