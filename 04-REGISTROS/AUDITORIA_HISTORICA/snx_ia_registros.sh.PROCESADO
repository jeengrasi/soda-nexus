...
mod_memoria() {
  ls -1 "$BASE" > "$MEM/logs_index_${MINISTERIO}.txt" 2>/dev/null || true
}
...
mod_contexto() {
  ...
  if [ ! -f "$FILE" ]; then
    ...
    echo "- Registro y auditoría del sistema" >> "$FILE"
  fi
}

mod_antiamnesia() {
  if [ ! -d "$BASE" ]; then
    log "ALERTA: falta carpeta de registros"
  fi
}
...
mod_estado() {
...

  {
    ...
    echo "  \"logs_index\": \"$( [ -f "$MEM/logs_index_${MINISTERIO}.txt" ] && wc -l < "$MEM/logs_index_${MINISTERIO}.txt" || echo 0 ) entradas\","
    echo "  \"engranaje\": $(cat "$MEM/engranaje_${MINISTERIO}.json"),"
    echo "  \"sync\": $(cat "$MEM/sync_${MINISTERIO}.json"),"
    echo "  \"archivos\": {"
    echo "    \"logs_index\": \"$( [ -f "$MEM/logs_index_${MINISTERIO}.txt" ] && echo ok || echo missing )\","
